...
zc-zerocode-tracker-load-error = The [todotracker] section in zerocode-config.toml could not be read, so it cannot be edited here ({ $error }). Fix the section by hand (or delete it to restore defaults), then reopen this pane.
